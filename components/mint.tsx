...
const handleMint = async () => {
    ...
    const handleApprove = async () => {
            ...
            if (connectedAddress) {
                ...
                .catch((error) => {console.log("this is an approve error", error);
                setMintState('approve error');
                setErrorMessage(error.message);})
            }
            else 
        ...
        switch (mintState) {
            ...
            case 'approve pending':
                return <div className="space-y-2 font-medium dark:text-white"><p>Hold tight, just a few more clicks.</p> <p>Approve spending {itemSum} WETH, once you've approved to spend the tokens you'll need to Mint.</p><button
                className="bg-gray-500 cursor-default text-gray-700 font-bold py-2 px-4 rounded"
                disabled
                ><LoadingSpinner/>Approving...</button>
                </div>;
            case 'approved':
                return <div className="space-y-2 font-medium dark:text-white"><p className="dark:text-green-300 text-green-700 font-bold">Approved</p> <p>Almost there! Keep clicking to mint those mitches!</p><button
                className="active:scale-90 transition-transform duration-100 bg-indigo-500 hover:bg-indigo-400 dark:bg-indigo-500 dark:hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded"
                onClick={() => {handleMint()}}
                >Mint</button>
                </div>;
            case 'native token':
                return <div className="space-y-2 font-medium dark:text-white"> <p>Let's Mint that Mitch!</p><button
                className="active:scale-90 transition-transform duration-100 bg-indigo-500 hover:bg-indigo-400 dark:bg-indigo-500 dark:hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded"
                onClick={() => {handleMint()}}
                >Mint</button>
                </div>;
            case 'approve error':
                return <div className="space-y-2 font-medium dark:text-white"><p>An error occured: <i>{errorMessage}</i>!  <br/> {approveTxHash? <p>Check this transaction data: {approveTxHash}</p> : '' }</p> <button 
                className="active:scale-90 transition-transform duration-100 bg-indigo-500 hover:bg-indigo-400 dark:bg-indigo-500 dark:hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded"
                onClick={() => {handleError()}}>Try Again</button></div>
            case 'mint error':
                return <div className="space-y-2 font-medium dark:text-white"><p>An error occured: <i>{errorMessage}</i>!  <br/> {mintTxHash ? <p>Check this transaction data: {mintTxHash}</p> : '' }</p> <button
                className="active:scale-90 transition-transform duration-100 bg-indigo-500 hover:bg-indigo-400 dark:bg-indigo-500 dark:hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded"
                onClick={() => {handleError()}}>Try Again</button></div>
            case 'mint pending':
                return <button
    ...
    return (
        ...
        <p className="text-gray-600">
            { parseFloat(formatEther(userBalance)) < itemSum ? <p className="text-red-600 text-lg">You don't have the required funds! Your current balance is {itemSum} </p> :  mintStateRender()}
        </p>
