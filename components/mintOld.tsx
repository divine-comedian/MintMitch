...
export const MintModal = ({ itemsArray, itemSum, isMintModal, isNativeMintEnabled }: MintItems) => {
    ...
    const approveAmount = BigNumber.from(parseEther(itemSum.toString()));; 
...
console.log("this is the native mint state on mint", isNativeMintEnabled)
...
const useMintTokensNative = (tokenId: number, mintAmount: number, value?: number) => {
    const {config, error} = usePrepareContractWrite({
        address: `0x${mintingContract.address}`,
        abi: mintingContract.abi,
...
const useMintBatchTokensNative =  (tokenId: number[], mintAmount: number[], value: number) => {
    const {config, error} = usePrepareContractWrite({
        address: `0x${mintingContract.address}`,
        abi: mintingContract.abi,
...
const mintTokensNative = useMintTokensNative(tokenId, 1, itemSum);
const mintBatchTokensNative = useMintBatchTokensNative(tokenBatchIds, tokenAmounts, itemSum);
...
useEffect(() => {
    ...
    else if (waitForApprove.isError) {
        setMintState('approve error')
        console.log("approve error", waitForApprove.isError)
        setMintTxHash(waitForApprove.data?.transactionHash)
    }
...
const handleMint = async () => {
    console.log("isNativeMintEnabled", isNativeMintEnabled)
    setMintState('mint pending');
    console.log("mintState", mintState)
    if (isNativeMintEnabled) {
        if (tokenBatchIds.length > 1) {
            if (mintBatchTokensNative.write) {
                mintBatchTokensNative.write();
            }
        } else {
            if (mintTokensNative.write) {
                mintTokensNative.write();
            }
        }
    } else {  
    if (tokenBatchIds.length > 1) {
        if (mintBatchTokens.write) {
             mintBatchTokens.write();
        }
    } else {
        if (mintTokens.write) {
             mintTokens.write();
