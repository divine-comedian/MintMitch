...
interface Item {
 ...
 const Store = () =>  { 
  ...
  const removeFromCart = useCallback((item: Item) => {
    setMintingCart((prevMintingCart) =>
    prevMintingCart.filter((i) => i.tokenID !== item.tokenID)
    );
  }, []);
  ...
  const {
   ...
   error: paymentTokenAddressError} = useContractRead({
    address: `0x${contractProps.address}`,
    abi: MintingContractJSON.abi,
  ...
  const {data: newUniqueTokens, isError: isUniqueTokensError, error: uniqueTokensError} = useContractRead({
    address: `0x${contractProps.address}`,
    abi: MintingContractJSON.abi,
    ...
    useEffect(() => {
     ...
     const MintingInfo = (props: MintingContractProps) => {
      ...
      switch (props.chainId) {
        ...
        message = (
          <div>
             You are currently connected to {props.name}, you'll need a bit of <a className='text-purple-600' href={contractProps.explorerLink + 'token/0x' + paymentTokenAddress} target="_blank" rel='noopener noreferrer'>{paymentTokenSymbol}</a> to mint a Mitch.
          </div>
        )
        case 137:
        return (
          <div>
            You can Mint any Mitchs you like on Gnosis Chain, Optimism and Polygon! You are currently connected to {props.name}, you'll need a bit of <a className='text-purple-600' href={contractProps.explorerLink + 'token/0x' + paymentTokenAddress} target="_blank" rel='noopener noreferrer'>{paymentTokenSymbol}</a> to mint a Mitch.
          </div>
            )
        }
        return message
      ...
      <Head>
        ...
        <Navbar isRightNetwork={rightNetwork} contractProps={contractProps} />
            ...
            <h1 className="text-3xl font-bold my-3">Here's the goods! Take your picks.</h1>
              <p>
                You can Mint any Mitchs you like on Gnosis Chain, Optimism and Polygon!
              </p>
              <p>
                 You are currently connected to <b>{contractProps.name}</b>, {isNativeMint ? <span>So you'll need a bit of ETH to mint some Mitchs.</span> : <span>so you'll need a bit of <a className='text-purple-600' href={contractProps.explorerLink + 'token/0x' + paymentTokenAddress.substring(2)} target="_blank" rel='noopener noreferrer'><b>{paymentTokenSymbol}</b></a> to mint a Mitch.</span>} 
              </p>
            ...
            <p><i>Also don't forget...</i> For every Mitch NFT you mint, you will receive 1 $MITCH token, these will be used to direct the flow of validator rewards once we reach the goal.</p>
            </div>
